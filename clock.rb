...
def docker_host
  @@docker_host ||= ENV['DOCKER_HOST'] ? ENV['DOCKER_HOST'] : "http://#{`route -n | grep 'UG[ \t]' | awk '{print $2}'`.strip}:2375"
end
...
begin
  ...
  @service_id = "#{aws_user_data['jockey_stack']}-#{aws_user_data['jockey_env']}"
rescue => e
...
every(10.seconds, 'update_consul')
every(10.seconds, 'report_self_to_health_check')
